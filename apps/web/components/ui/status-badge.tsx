...
function toneFor(status: string): Tone {
  if (["approved", "approval_ready", "auto_approved", "clear", "completed", "exported", "erp_ready", "paid"].includes(status)) {
    return "success";
  }
  if (
    ["blocked", "on_hold", "missing_po", "review_required", "warning", "pending", "high", "critical", "likely_duplicate", "duplicate"].includes(
      status
    )
  ) {
    return "warning";
  }
  if (["rejected", "failed", "overdue", "danger"].includes(status)) {
    return "danger";
  }
