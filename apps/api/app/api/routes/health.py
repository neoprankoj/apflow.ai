    ...
    return ProductReadinessResponse(
        ...
        demo_ready=_readiness_level(
            ...
            required_keys=[
                ...
                "demo_seed_profiles_available",
                "pilot_data_packs_available",
            ],
...
def _product_readiness_checks() -> list[ProductReadinessCheck]:
    ...
    return [
        ...
        _check(
            "demo_seed_profiles_available",
            "Demo seed profiles available",
            "pass",
            "demo",
            "Repeatable seed profiles are available for AP manager, vendor self-service, Priority, compliance, analytics, and clean demos.",
        ),
        _check(
            "controlled_demo_reset_available",
            "Controlled demo reset available",
            "pass" if settings.allow_demo_reset and settings.app_env != "production" else "warning",
            "demo",
            "Demo seeding can run in this non-production environment."
            if settings.allow_demo_reset and settings.app_env != "production"
            else "Demo seeding is currently disabled or unavailable, which is safer outside active demo setup.",
            "Temporarily enable ALLOW_DEMO_RESET only on private staging, seed, then disable it again.",
        ),
        _check(
            "pilot_data_packs_available",
            "Pilot data packs available",
            "pass",
            "operations",
            "Deterministic pilot/demo data packs can populate AP workflow, vendor, Priority, compliance, analytics, and usage examples.",
        ),
        _check(
            "ocr_provider_configured",
        ...
        _check("usage_metering_foundation_available", "Usage metering foundation", "pass", "commercial", "Tenant-scoped usage events and warn-only plan summaries are available.", "Connect a billing provider only after pilot terms are defined."),
        _check("billing_provider_connected", "Billing provider connected", "fail", "commercial", "No real billing provider is connected.", "Connect Stripe or another billing provider after commercial packaging is ready."),
        _check("plan_limit_enforcement_available", "Plan limit enforcement", "warning", "commercial", "Plan limits are visibility-only and warn-only; workflow is not blocked.", "Keep limits warn-only until customer contracts and overage policy are approved."),
        _check("customer_subscription_management_available", "Customer subscription management", "fail", "commercial", "Customer subscription management is not configured.", "Add subscription lifecycle only when real billing is introduced."),
        ...
        _check("production_demo_reset_blocked", "Production demo reset blocked", "pass" if settings.app_env != "production" or not settings.allow_demo_reset else "fail", "production", "Demo reset and seed routes are blocked by production guardrails.", "Never enable ALLOW_DEMO_RESET in production."),
