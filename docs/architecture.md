...
- `PriorityODataAdapter`, an experimental real-connector foundation that validates Priority OData configuration/service-root access and consumes tenant-scoped mapping JSON for vendor, PO, and invoice-export shapes.
...
ERP sync logs and external references are tenant-scoped. Real ERP adapters can be added by implementing `ERPAdapterProtocol` without changing `ERPConnectorAgent`. Priority mapping JSON is stored inside the existing tenant-scoped `ERPConnectionConfig.config` payload rather than hardcoding customer-specific forms. Priority real-readiness checks are separate from sync: the default endpoint inspects local config only, while the remote drill performs GET-only service-root and `$metadata` checks when real mode, credentials, and the read-only gate are configured. Priority vendor/PO sync preview reuses the same mapping helpers to transform deterministic sample rows by default, or explicitly requested read-only Priority OData rows when real mode, credentials, mapping, and `PRIORITY_ERP_READ_ONLY_FETCH_ENABLED=true` are all present. Priority import-plan helpers compare those mapped preview rows against existing APFlow vendors and purchase orders, classifying each row as create, update, skip, or conflict without writing data. Controlled Priority import then regenerates the plan server-side and writes only selected rows into APFlow after explicit confirmation; it links ERP external references and records audit events, but never writes to Priority. Imported-record views are read-only projections over APFlow vendors/POs, ERP external references, and audit events so operators can verify what exists after import. Real Priority invoice export builds a mapped payload preview first and remains write-disabled by default.
