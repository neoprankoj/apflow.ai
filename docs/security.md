...
- Notification delivery supports mock-only internal delivery history plus Email, Slack, and Teams placeholders. The real-provider gate in [notification_provider_readiness.md](notification_provider_readiness.md) must stay secret-safe and mock-first; placeholder providers must not send externally until server-only secrets, sender-domain authentication, retries, Domain + HTTPS, and support ownership are configured.
...
- Legal/privacy/data-handling materials are draft readiness documents only. They are not legal advice, not counsel-approved terms, and not GDPR/SOC 2/ISO compliance claims. Use [legal_privacy_data_pack.md](legal_privacy_data_pack.md), [customer_data_handling_summary.md](customer_data_handling_summary.md), and [pilot_terms_outline.md](pilot_terms_outline.md) before real customer pilot discussions.
