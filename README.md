...
ERP integration is explicit through `/erp/*` endpoints. Mock adapters are available for Priority, Odoo, and Zoho Books. Priority also has an experimental real-connector foundation that can validate configuration and OData service-root reachability when `PRIORITY_ERP_MODE=real`. Real mode now supports tenant-scoped Priority mapping configuration for vendor, purchase-order, and invoice-export shapes; writes remain disabled by default with `PRIORITY_ERP_ENABLE_WRITES=false`. Priority sync preview, import plan, controlled APFlow-only import, and imported-record visibility are available for safe operator validation without writing to Priority. Real Priority read-only fetch is disabled by default with `PRIORITY_ERP_READ_ONLY_FETCH_ENABLED=false`; sample previews remain the default. The Priority readiness endpoint and Admin checklist show missing real-credential/config items and can run a gated GET-only service-root/metadata drill without fetching entity data.
...
OCR.space can be enabled with `OCR_PROVIDER=ocr_space` and `OCR_SPACE_API_KEY`. It sends uploaded PDF/image bytes to OCR.space and maps `ParsedText` into the shared field-confidence and human-review workflow with conservative parsing. OCR.space is general OCR, not specialized invoice intelligence, so unclear invoices may safely stop at `review_required`. APFlow validates PDF/PNG/JPG file signatures before OCR.space calls, surfaces invalid-file and OCR engine errors clearly, and can retry a configured fallback engine with `OCR_SPACE_ENABLE_ENGINE_FALLBACK=true` and `OCR_SPACE_FALLBACK_ENGINE=2`.
...
Vendor portal access now has a production-shaped lifecycle foundation: admin-created supplier access, browser-friendly `/vendor` links, hashed tokens, one-time raw token display, expiration, revocation, rotation, last-used tracking, audit events, strict vendor-safe invoice/payment filtering, and a deterministic vendor payment-status chatbot. Real email invitation delivery is not connected yet; see `docs/vendor_access.md` and `docs/vendor_payment_chatbot.md`.
