...
ERP integration is explicit through `/erp/*` endpoints. Mock adapters are available for Priority, Odoo, and Zoho Books. Priority also has an experimental real-connector foundation that can validate configuration and OData service-root reachability when `PRIORITY_ERP_MODE=real`; live vendor/PO/export work still requires tenant-specific mapping configuration.
